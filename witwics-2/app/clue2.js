...
function returnIndex(arr, str) {
  let foundIndex = arr.indexOf(str)
  return foundIndex
}
...
function findById(id) {
  let found = staff.find(staff => staff.id === id)
  if (found) {
    return found
  } else {
...
function bandMemberDetails(name) {
  ...
  return band += `${found.name} is in the band and plays the ${found.instrument}`
}
